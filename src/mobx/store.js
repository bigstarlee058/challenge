...
export class Store {
    ...
    return new Promise((resolve, reject) => {
      ...
      videoElement.onloadedmetadata = async () => {
        ...
        if (!existingElement) {
          ...
          // Add aligned audio track
          const audioId = `audio-${videoId}`;
          const audioRow = this.maxRows;
          this.maxRows++;
          ...
          this.addEditorElement({
            ...
            timeFrame: {
              start: this.editorElements.find(el => el.id === videoId).timeFrame.start,
              end: this.editorElements.find(el => el.id === videoId).timeFrame.end,
            },
            row: audioRow,
            properties: {
          ...
          // Trigger Redux sync after adding video and audio elements
          if (window.dispatchSaveTimelineState && !this.isUndoRedoOperation) {
            window.dispatchSaveTimelineState(this);
          }
          // Show the new video
          ...
          videoElement.muted = true; // Mute video since we have separate audio track
  ...
  splitVideoElement(item, splitPoint) {
    runInAction(() => {
      const index = this.editorElements.findIndex(el => el.id === item.id);
      if (index === -1) return;
      ...
      const oldEnd = element.timeFrame.end;

      // Update the first half
      element.timeFrame.end = splitPoint;

      // Update the second half
      newElement.timeFrame.start = splitPoint;
      newElement.timeFrame.end = oldEnd;
      const offsetChange = splitPoint - originalStart;
      newElement.properties.videoOffset =
        (element.properties.videoOffset || 0) + offsetChange;
      newElement.properties.elementId = `video-${newId}`;

      // Update thumbnails for both parts
      if (element.properties.thumbnails) {
        const oldTotalDuration = oldEnd - originalStart;
        const splitRatio = (splitPoint - originalStart) / oldTotalDuration;

        const thumbnailCount = element.properties.thumbnails.length;
        const splitIndex = Math.max(
          1,
          Math.floor(thumbnailCount * splitRatio)
        );

        const firstPartThumbnails = element.properties.thumbnails.slice(
          0,
          splitIndex
        );
        const secondPartThumbnails = element.properties.thumbnails.slice(
          splitIndex
        );

        element.properties.thumbnails = firstPartThumbnails;
        newElement.properties.thumbnails = secondPartThumbnails;
      }

      // Create new HTML video element
      const videoElement = document.createElement('video');
      videoElement.id = newElement.properties.elementId;
      ...
      // Clear fabricObject for the new element so it's recreated in refreshElements
      newElement.fabricObject = null;
      if (newElement.properties.imageObject) {
        newElement.properties.imageObject = null;
      }

      this.editorElements.splice(index + 1, 0, newElement);
      this.refreshElements();
  ...
  splitAudioElement(item, splitPoint) {
    runInAction(() => {
      ...
      newElement.timeFrame.end = oldEnd;
      const offsetChange = splitPoint - originalStart;
      newElement.properties.audioOffset =
        (element.properties.audioOffset || 0) + offsetChange;
      newElement.properties.elementId = `audio-${newId}`;

      // Create new HTML audio element
      const audioElement = document.createElement('audio');
      audioElement.id = newElement.properties.elementId;
      ...
      // Clear fabricObject for the new element if any
      newElement.fabricObject = null;
  ...
  splitImageElement(item, splitPoint) {
    runInAction(() => {
      ...
      // Clear fabricObject for the new element so it's recreated
      newElement.fabricObject = null;
      if (newElement.properties.imageObject) {
        newElement.properties.imageObject = null;
      }
  ...
  async removeSilenceLocally(audioId, settings = {}) {
    const audioElement = this.editorElements.find(
      el => el.id === audioId && el.type === 'audio'
    );

    if (!audioElement) {
      console.error('No audio element found for silence removal');
      return;
    }

    const audioUrl = audioElement.properties?.src || audioElement.src;
    if (!audioUrl) {
      console.error('No audio URL found for silence removal');
      return;
    }
    ...
    try {
      ...
      const threshold = settings.threshold || 0.01;
      const minSilenceDuration = settings.minSilenceDuration || 0.5;

      const nonSilentRegions = await audioProcessor.detectSilence(
        audioUrl,
        threshold,
        minSilenceDuration
      );
      ...
      runInAction(() => {
        const index = this.editorElements.findIndex(el => el.id === audioId);
        if (index === -1) return;

        const originalStart = audioElement.timeFrame.start;
        let currentTimelinePos = originalStart;

        const newElements = nonSilentRegions.map((region, i) => {
          const newId = i === 0 ? audioId : uuidv4();
          const duration = (region.end - region.start) * 1000;
          
          const newElement = JSON.parse(JSON.stringify(audioElement));
          newElement.id = newId;
          newElement.isLoading = false;
          newElement.timeFrame = {
            start: currentTimelinePos,
            end: currentTimelinePos + duration,
          };
          
          // Calculate offset: original audioOffset + region start time
          const originalOffset = audioElement.properties.audioOffset || 0;
          newElement.properties.audioOffset = originalOffset + (region.start * 1000);
          newElement.properties.elementId = `audio-${newId}`;
          
          // Create new HTML audio element for additional segments
          if (i > 0) {
            const htmlAudio = document.createElement('audio');
            htmlAudio.id = newElement.properties.elementId;
        ...
        // Replace the original element and insert others
        this.editorElements.splice(index, 1, ...newElements);
        this.refreshElements();
      ...
      console.error('Error removing silence locally:', error);
      runInAction(() => {
        audioElement.isLoading = false;
      });
    }
