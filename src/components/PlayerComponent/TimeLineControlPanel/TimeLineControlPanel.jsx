...
const TimeLineControlPanel = ({
  ...
  const handleRemoveSilenceApply = async (settings, audioId) => {
    ...
    try {
      // Use local silence removal
      const thresholdLinear = Math.pow(
        10,
        (parseFloat(settings.startThreshold) || -45) / 20
      );
      const minSilenceDuration = parseFloat(settings.stopDuration) || 0.5;

      await store.removeSilenceLocally(audioId, {
        threshold: thresholdLinear,
        minSilenceDuration: minSilenceDuration,
      });

      console.log('✅ Local silence removal completed');
      setIsRemoveSilenceVisible(false);
    } catch (error) {
      console.error('❌ Local silence removal failed:', error);
    } finally {
